 ...
 * @author messoudi
 */
public class ReclamationEditForm extends SideMenuBaseForm {

    public ReclamationEditForm(int id, Resources res/*, int idcommande*/) {
        ...
        Reclamation r = ServiceReclamation.getInstance().getReclamation(id);
        TextField type = new TextField(r.getType(), "Report Subject", 20, TextField.USERNAME);
        TextArea description = new TextArea(r.getDescription(), 5, 5, TextField.USERNAME);
       ...
       ComboBox <String> destinataire = new ComboBox <> (vectorDest);
        ...
        addButton.addActionListener(e -> {
            if (Dialog.show("Succes", "The report will be modified", "Ok", "Cancel")) {
                ServiceReclamation.getInstance().editReclamation(new Reclamation(type.getText(), description.getText(),destinataire.getSelectedItem().toString(),SessionManager.getId()));
